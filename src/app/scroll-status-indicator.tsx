...
// 정다각형의 꼭짓점 계산 (외접원 기준)
const getPolygonPoints = (sides: number, radius: number, centerX: number, centerY: number): string => {
...
const getShapeElement = (sides: number, size: number, isActive: boolean, reducedMotion: boolean) => {
...
export default function ScrollStatusIndicator() {
  ...
  // 다음 도형 계산 로직
  const getNextSides = (current: number): number => {
  ...
  // 도형 이름 반환 (디버깅/접근성용)
  const getShapeName = (sides: number): string => {
  ...
  const updateScrollProgress = useCallback(() => {
    ...
    // 실제 스크롤이 발생했는지 확인
    const hasScrolled = Math.abs(scrollTop - lastScrollYRef.current) > 1;
  ...
  const animationLoop = useCallback(() => {
    ...
    const shouldContinue = isVisible && (
      ...
      Date.now() - lastScrollTimeRef.current < 100 // 100ms 내 스크롤
    );
  ...
  const throttledScroll = useThrottledScroll(updateScrollProgress, 16);
  ...
  }, [throttledScroll, updateScrollProgress, isMounted]);
